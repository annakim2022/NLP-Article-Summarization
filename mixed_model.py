...
def title_biased_sentence_scores(sentence_list, word_frequencies, title_term_frequencies, ner_mods, tfidf_mods):
    sentence_scores = {}
            ...
            if word in word_frequencies.keys():
                if len(sent.split(' ')) < 30:
                    ...
                    if sent not in sentence_scores.keys():
                        sentence_scores[sent] = word_frequencies[word] + (title_biased_modifier*.5) + (ner_mods[sent]*.5) + (tfidf_mods[sent]*.5)
                        num_title_terms += 1
                    else:
                        sentence_scores[sent] += word_frequencies[word] + (title_biased_modifier*.5) + (ner_mods[sent]*.5) + (tfidf_mods[sent]*.5)
                        num_title_terms += 1
...
def generate_summary(url, ner_mods, tfidf_mods):
    ...
    return title_biased_sentence_scores(sentences, word_frequencies, title_term_frequencies, ner_mods, tfidf_mods)